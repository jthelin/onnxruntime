...
for onnx_version in "${!version2tag[@]}"; do
  ONNX_VERSION_TAG=${version2tag[$onnx_version]}
  DATA_DIR="/data/onnx/${ONNX_VERSION_TAG}"
  ...
  mkdir -p "${DATA_DIR}"
  backend-test-tools generate-data -o "${DATA_DIR}"
  echo "$onnx_version:${ONNX_VERSION_TAG}" >> /data/onnx/version2tag
done
